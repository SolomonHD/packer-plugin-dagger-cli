...
.PHONY: dev build test install-packer-sdc plugin-check testacc generate dev-test prod prod-install
...
## prod: Build production plugin without prerelease suffix, run describe test, and clean up
## This target creates production-ready builds suitable for release verification:
## - Reads version from version/VERSION (single source of truth)
## - Sets VersionPrerelease to empty string (no suffix)
## - Derives git host/org from .git and lowercases them
## - Uses PACKER_PLUGIN_BIN environment variable
## - Runs describe test to verify version reporting matches VERSION exactly
## - Moves binary to /tmp for cleanup
prod:
	...
	echo "    Binary: ${PACKER_PLUGIN_BIN}"; \
	echo "==> Building ${PACKER_PLUGIN_BIN}..."; \
	go build -ldflags="-X '${PLUGIN_FQN}/version.Version=$$VERSION' -X '${PLUGIN_FQN}/version.VersionPrerelease='" -o ${PACKER_PLUGIN_BIN}; \
	BUILD_EXIT=$$?; \
	if [ $$BUILD_EXIT -ne 0 ]; then \
		echo "ERROR: Build failed with exit code $$BUILD_EXIT"; \
		exit $$BUILD_EXIT; \
	...
	if [ "$$REPORTED_VERSION" != "$$VERSION" ]; then \
		echo "ERROR: Reported version ($$REPORTED_VERSION) does not match expected version ($$VERSION)"; \
		mv ${PACKER_PLUGIN_BIN} /tmp/${PACKER_PLUGIN_BIN} 2>/dev/null || true; \
		exit 1; \
	fi; \
	echo "==> Cleaning up..."; \
	...
	echo "    Binary: /tmp/${PACKER_PLUGIN_BIN}"; \
	echo ""; \
	echo "To install for local use, run: make prod-install"

## prod-install: Build and install production plugin for local testing (skips describe test)
## Use this when you want to quickly install a production build to your Packer plugins directory
## without the -dev suffix for testing release-ready functionality
prod-install:
	@echo "==> Building and installing production plugin..."
	@VERSION=$$(cat version/VERSION | tr -d '[:space:]'); \
	go build -ldflags="-X '${PLUGIN_FQN}/version.Version=$$VERSION' -X '${PLUGIN_FQN}/version.VersionPrerelease='" -o ${PACKER_PLUGIN_BIN}; \
	packer plugins install --path ${PACKER_PLUGIN_BIN} "$$(echo "${PLUGIN_FQN}" | sed 's/packer-plugin-//')"
